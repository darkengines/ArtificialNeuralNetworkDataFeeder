...
namespace ArtificialNeuralNetworkDataFeeder.Core {
    public class DataProvider {
        ...
        protected double[] BuildTrainingData (Datum[] data, out uint dataCount) {
            ...
            var limit = dataCount - InputDataPickers.Count;
            ...
            while (i < limit) {
                ...
                while (j < dataPickersCount) {
                    ...
                    while (k < compiledInputsCount) {
                        compiledInputs[k] = dataPicker.Compiler.Compile(data[dataPicker.Index + i]);
                        k++;
                    }
                    var processedValue = dataPicker.Indicator.Process(compiledInputs, dataPicker.Indicator.IndexMinimum);
